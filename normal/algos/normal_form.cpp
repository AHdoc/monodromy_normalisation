pair<bool,Tuple<Ga3b2>> get_normal_form(int pa,int qa,int nb,int p,int q){
	...
	while(q>=6){ // ba ab ba ab ba ab = 1
		g0=bullet(Tuple<Ga3b2>({t0,t2,t0,t2,t0,t2}),g0);
		q-=6;
	}
	while(p>=6){ // aab baa aab baa aab baa = 1
		g0=bullet(Tuple<Ga3b2>({s0,s2,s0,s2,s0,s2}),g0);
		p-=6;
	}
	if(qa==1 && p==2){
		g0=bullet(Tuple<Ga3b2>({a2,s0,s2}),g0);
		qa--; p-=2;
	}else if(pa==1 && q==2){
		g0=bullet(Tuple<Ga3b2>({a,t2,t0}),g0);
		pa--; q-=2;
	}else if(pa==1 && p==4){
		g0=bullet(Tuple<Ga3b2>({a,s0,s0,s2,s0}),g0);
		pa--; p-=4;
	}else if(qa==1 && q==4){
		g0=bullet(Tuple<Ga3b2>({a2,t0,t2,t0,t0}),g0);
		qa--; q-=4;
	}else if(nb==1 && p==3){ // b aab baa aab
		g0=bullet(Tuple<Ga3b2>({b,s0,s2,s0}),g0);
		nb--; p-=3;
	}else if(nb==1 && q==3){ // b ba ab ba
		g0=bullet(Tuple<Ga3b2>({b,t0,t2,t0}),g0);
		nb--; q-=3;
	}else if(pa==1 && nb==1 && p==1){
		g0=bullet(Tuple<Ga3b2>({a,b,s2}),g0);
		pa--; nb--; p--;
	}else if(qa==1 && nb==1 && q==1){
		g0=bullet(Tuple<Ga3b2>({a2,b,t0}),g0);
		qa--; nb--; q--;
	}else if(pa==1 && nb==1 && q==5){
		g0=bullet(Tuple<Ga3b2>({a,t2,t0,b,t0,t2,t0}),g0);
		pa--; nb--; q-=5;
	}else if(qa==1 && nb==1 && p==5){
		g0=bullet(Tuple<Ga3b2>({a2,s0,s2,b,s0,s2,s0}),g0);
		qa--; nb--; p-=5;
	}else if(pa==2 && p==2){
		g0=bullet(Tuple<Ga3b2>({a,a,s0,s2}),g0);
		pa-=2; p-=2;
	}else if(qa==2 && q==2){
		g0=bullet(Tuple<Ga3b2>({a2,a2,t2,t0}),g0);
		qa-=2; q-=2;
	}else if(qa==2 && p==4){
		g0=bullet(Tuple<Ga3b2>({a2,a2,s0,s0,s2,s0}),g0);
		qa-=2; p-=4;
	}else if(pa==2 && q==4){
		g0=bullet(Tuple<Ga3b2>({a,a,t0,t2,t0,t0}),g0);
		pa-=2; q-=4;
	}else if(qa==2 && nb==1 && p==1){
		g0=bullet(Tuple<Ga3b2>({a2,a2,b,s2}),g0);
		qa-=2; nb--; p--;
	}else if(pa==2 && nb==1 && q==1){
		g0=bullet(Tuple<Ga3b2>({a,a,b,t0}),g0);
		pa-=2; nb--; q--;
	}else if(qa==2 && nb==1 && q==5){
		g0=bullet(Tuple<Ga3b2>({a2,a2,t2,t0,b,t0,t2,t0}),g0);
		qa-=2; nb--; q-=5;
	}else if(pa==2 && nb==1 && p==5){
		g0=bullet(Tuple<Ga3b2>({a,a,s0,s2,b,s0,s2,s0}),g0);
		pa-=2; nb--; p-=5;
	}
